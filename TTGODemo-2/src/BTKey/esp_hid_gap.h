...
//#include "BTKey/bluetoothkeyboard.h"
...
static const char *TAGX = "ESP_HID_GAP";
const char *ble_addr_type_str(esp_ble_addr_type_t ble_addr_type)
{
...
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod, esp_bt_uuid_t *uuid, uint8_t *name, uint8_t name_len, int rssi)
{
    ...
    if (r) {
        ...
        if (r->name == NULL && name && name_len) {
            ...
            if (name_s == NULL) {
                ESP_LOGE(TAGX, "Malloc result name failed!");
                return;
            }
    ...
    if (r == NULL) {
        ESP_LOGE(TAGX, "Malloc bt_hidh_scan_result_t failed!");
        return;
    }
...
static void add_ble_scan_result(esp_bd_addr_t bda, esp_ble_addr_type_t addr_type, uint16_t appearance, uint8_t *name, uint8_t name_len, int rssi)
{
    if (find_scan_result(bda, ble_scan_results)) {
        ESP_LOGW(TAGX, "Result already exists!");
        return;
    }
    esp_hid_scan_result_t *r = (esp_hid_scan_result_t *)malloc(sizeof(esp_hid_scan_result_t));
    if (r == NULL) {
        ESP_LOGE(TAGX, "Malloc ble_hidh_scan_result_t failed!");
        return;
    }
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: {
        ESP_LOGV(TAGX, "BT GAP DISC_STATE %s", (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STARTED) ? "START" : "STOP");
    ...
    case ESP_BT_GAP_KEY_NOTIF_EVT:
        ESP_LOGI(TAGX, "BT GAP KEY_NOTIF passkey:%d", param->key_notif.passkey);
        break;
    default:
        ESP_LOGV(TAGX, "BT GAP EVENT %s", bt_gap_evt_str(event));
        break;
    }
}
...
static esp_err_t start_bt_scan(uint32_t seconds)
{
    ...
    if ((ret = esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, (int)(seconds / 1.28), 0)) != ESP_OK) {
        ESP_LOGE(TAGX, "esp_bt_gap_start_discovery failed: %d", ret);
        return ret;
    }
    return ret;
}
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT: {
        ESP_LOGV(TAGX, "BLE GAP EVENT SCAN_PARAM_SET_COMPLETE");
        SEND_BLE_CB();
        break;
    }
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_CMPL_EVT:
            ESP_LOGV(TAGX, "BLE GAP EVENT SCAN DONE: %d", scan_result->scan_rst.num_resps);
            SEND_BLE_CB();
            break;
        default:
            break;
    ...
    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT: {
        ESP_LOGV(TAGX, "BLE GAP EVENT SCAN CANCELED");
        break;
    }
    ...
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
        ESP_LOGV(TAGX, "BLE GAP ADV_DATA_SET_COMPLETE");
        break;

    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT:
        ESP_LOGV(TAGX, "BLE GAP ADV_START_COMPLETE");
        break;
    ...
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
        if (!param->ble_security.auth_cmpl.success) {
            ESP_LOGE(TAGX, "BLE GAP AUTH ERROR: 0x%x", param->ble_security.auth_cmpl.fail_reason);
        } else {
            ESP_LOGI(TAGX, "BLE GAP AUTH SUCCESS");
        }
        break;

    case ESP_GAP_BLE_KEY_EVT: //shows the ble key info share with peer device to the user.
        ESP_LOGI(TAGX, "BLE GAP KEY type = %s", esp_ble_key_type_str(param->ble_security.ble_key.key_type));
        break;

    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT: // ESP_IO_CAP_OUT
        // The app will receive this evt when the IO has Output capability and the peer device IO has Input capability.
        // Show the passkey number to the user to input it in the peer device.
        ESP_LOGI(TAGX, "BLE GAP PASSKEY_NOTIF passkey:%d", param->ble_security.key_notif.passkey);
        break;

    case ESP_GAP_BLE_NC_REQ_EVT: // ESP_IO_CAP_IO
        // The app will receive this event when the IO has DisplayYesNO capability and the peer device IO also has DisplayYesNo capability.
        // show the passkey number to the user to confirm it with the number displayed by peer device.
        ESP_LOGI(TAGX, "BLE GAP NC_REQ passkey:%d", param->ble_security.key_notif.passkey);
    ...
    case ESP_GAP_BLE_PASSKEY_REQ_EVT: // ESP_IO_CAP_IN
        ...
        ESP_LOGI(TAGX, "BLE GAP PASSKEY_REQ");
        //esp_ble_passkey_reply(param->ble_security.ble_req.bd_addr, true, 1234);
        break;

    case ESP_GAP_BLE_SEC_REQ_EVT:
        ESP_LOGI(TAGX, "BLE GAP SEC_REQ");
    ...
    default:
        ESP_LOGV(TAGX, "BLE GAP EVENT %s", ble_gap_evt_str(event));
        break;
    }
}
...
static esp_err_t init_ble_gap(void)
{
    ...
    if ((ret = esp_ble_gap_register_callback(ble_gap_event_handler)) != ESP_OK) {
        ESP_LOGE(TAGX, "esp_ble_gap_register_callback failed: %d", ret);
        return ret;
    }
    return ret;
}
...
static esp_err_t start_ble_scan(uint32_t seconds)
{
    ...
    if ((ret = esp_ble_gap_set_scan_params(&hid_scan_params)) != ESP_OK) {
        ESP_LOGE(TAGX, "esp_ble_gap_set_scan_params failed: %d", ret);
        return ret;
    }
    WAIT_BLE_CB();

    if ((ret = esp_ble_gap_start_scanning(seconds)) != ESP_OK) {
        ESP_LOGE(TAGX, "esp_ble_gap_start_scanning failed: %d", ret);
        return ret;
    }
    return ret;
}
...
static esp_err_t init_low_level(uint8_t mode)
{
    ...
    if (mode & ESP_BT_MODE_CLASSIC_BT) {
        ...
        if (ret) {
            ESP_LOGE(TAGX, "esp_bt_controller_mem_release failed: %d", ret);
            return ret;
        }
    }
    ret = esp_bt_controller_init(&bt_cfg);
    if (ret) {
        ESP_LOGE(TAGX, "esp_bt_controller_init failed: %d", ret);
        return ret;
    }

    ret = esp_bt_controller_enable(mode);
    if (ret) {
        ESP_LOGE(TAGX, "esp_bt_controller_enable failed: %d", ret);
        return ret;
    }

    ret = esp_bluedroid_init();
    if (ret) {
        ESP_LOGE(TAGX, "esp_bluedroid_init failed: %d", ret);
        return ret;
    }

    ret = esp_bluedroid_enable();
    if (ret) {
        ESP_LOGE(TAGX, "esp_bluedroid_enable failed: %d", ret);
        return ret;
    }
...
esp_err_t esp_hid_gap_init(uint8_t mode)
{
    ...
    if (!mode || mode > ESP_BT_MODE_BTDM) {
        ESP_LOGE(TAGX, "Invalid mode given!");
        return ESP_FAIL;
    }

    if (bt_hidh_cb_semaphore != NULL) {
        ESP_LOGE(TAGX, "Already initialised");
        return ESP_FAIL;
    }

    bt_hidh_cb_semaphore = xSemaphoreCreateBinary();
    if (bt_hidh_cb_semaphore == NULL) {
        ESP_LOGE(TAGX, "xSemaphoreCreateMutex failed!");
        return ESP_FAIL;
    }
...
esp_err_t esp_hid_scan(uint32_t seconds, size_t *num_results, esp_hid_scan_result_t **results)
{
    if (num_bt_scan_results || bt_scan_results || num_ble_scan_results || ble_scan_results) {
        ESP_LOGE(TAGX, "There are old scan results. Free them first!");
        return ESP_FAIL;
    }
